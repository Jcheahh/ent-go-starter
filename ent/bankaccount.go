...
type BankAccount struct {
	...
	shop_bank_accounts *int
	user_bank_accounts *int
...
func (*BankAccount) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case bankaccount.FieldID, bankaccount.FieldXid:
			values[i] = new(sql.NullInt64)
		case bankaccount.ForeignKeys[0]: // shop_bank_accounts
			values[i] = new(sql.NullInt64)
		case bankaccount.ForeignKeys[1]: // user_bank_accounts
			values[i] = new(sql.NullInt64)
		default:
	...
	for i := range columns {
		...
		case bankaccount.FieldXid:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				...
				ba.Xid = int(value.Int64)
			}
		case bankaccount.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for edge-field shop_bank_accounts", value)
			} else if value.Valid {
				ba.shop_bank_accounts = new(int)
				*ba.shop_bank_accounts = int(value.Int64)
			}
		case bankaccount.ForeignKeys[1]:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for edge-field user_bank_accounts", value)
			} else if value.Valid {
				ba.user_bank_accounts = new(int)
				*ba.user_bank_accounts = int(value.Int64)
			}
		default:
...
func (ba *BankAccount) String() string {
	...
	builder.WriteString(fmt.Sprintf("%v", ba.Xid))
	builder.WriteByte(')')
