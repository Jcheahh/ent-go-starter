...
func (cc *ChatCreate) SetXid(i int) *ChatCreate {
	cc.mutation.SetXid(i)
	return cc
}
...
func (cc *ChatCreate) check() error {
	if _, ok := cc.mutation.Xid(); !ok {
		return &ValidationError{Name: "xid", err: errors.New(`ent: missing required field "Chat.xid"`)}
	}
	return nil
}
...
func (cc *ChatCreate) createSpec() (*Chat, *sqlgraph.CreateSpec) {
	...
	if value, ok := cc.mutation.Xid(); ok {
